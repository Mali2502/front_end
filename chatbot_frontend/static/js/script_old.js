...
function loadChat(chatID) {
    ...
    fetch(`http://bot.aqarchain.com/chat/${chatID}`, {
        method: 'GET',
        credentials: 'include'
    })
...
function sendQuery(query) {
    ...
    fetch('http://bot.aqarchain.com/chat', { 
        method: 'POST',
        credentials: 'include', 
        headers: {
    ...
    .then(data => {
        if (data.response) {
            ...
            if (authenticatedBool === 1) {
                fabButton.style.display = 'none'; 
            } else if (authenticatedBool === 2) {
...
function clearChat() {
    ...
    fetch('http://bot.aqarchain.com/clear_chat', {
        method: 'GET',
        credentials: 'include'
    })
    .then(response => response.json())
...
function updateChatName(chatId, button) {
    ...
    fetch(`http://bot.aqarchain.com/get_chat_name/${chatId}`, {
        method: 'GET',
    ...
    .then(chatData => {
        ...
        showInputModal(function(newName) {
            fetch('http://bot.aqarchain.com/update_chat_name', {
                method: 'POST',
                credentials: 'include', 
...
function deleteChat(chatId, button) {
    showConfirmModal('Are you sure you want to delete this chat permanently?', function() {
        fetch(`http://bot.aqarchain.com/delete_chat/${chatId}`, {
            method: 'POST',
            credentials: 'include' // Include credentials (cookies) in the request
        })
        .then(response => response.json())
...
function deleteAllChats() {
    showConfirmModal('Are you sure you want to delete ALL chats permanently?', function() {
        fetch(`http://bot.aqarchain.com/delete_all`, { 
            method: 'POST',
            credentials: 'include' 
        })
            .then(response => response.json())
...
function populateSidebar() {
    fetch('http://bot.aqarchain.com/chat_ids', {
        method: 'GET',
        credentials: 'include' 
    })
        .then(response => response.json())
        .then(data => {
            ...
            function createButton(chat) {
                ...
                fetch(`http://bot.aqarchain.com/get_chat_name/${chat.id}`, {
                    method: 'GET',
                    credentials: 'include' // Include credentials (cookies) in the request
                })
                    .then(response => response.json())
